...
def get_calibs_from_xlsx(column_names):
    ...
    => pandas data frame
    """
    table = pd.read_excel('../raw-DAQ-files/sensorCalib.xlsx',  
...
def get_calibrated_data(path, calib_table):
    ...
    for sensor in dic:
        ...
        """
        Potential refactor to make extensible based on calib table
        # Apply calibration values to corresponding sensor and channel
            
        
        for parameter in calib_table.columns:
            sensor = parameter.split('/')[0]
            channel = int(parameter.split('/')[1])
            dic[sensor][channel] = dic[sensor][channel].map(
