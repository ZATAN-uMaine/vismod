...
class CalibrationManager:
    def __init__(
        self, calibration_file_path, select_cols=None, select_index=None
    ):
        self.calibration_file_path = calibration_file_path
        self.parameter_columns = select_cols
        self.calib_table = self.get_calibs_from_local_xlsx(
            select_cols, select_index
        )
    ...
    def rename_cols(self, new_dict):
        ...
        if not isinstance(new_dict, dict):
            return
        self.calib_table.rename(columns=new_dict, inplace=True)
    ...
    def apply_calibration_factors(self, tdms_dict):
        for parameter in self.calib_table.columns:
            ...
            tdms_dict[sensor][channel] = tdms_dict[sensor][channel].map(
                lambda f: f
                * self.calib_table[parameter][sensor + f"/{channel}"]
            )
        return tdms_dict
